...
class CenterClubsStep(BaseComponent):
    ...
    CLUB_CHECKBOXES: Locator = (By.CSS_SELECTOR, "#clubs input[type='checkbox']")
    ...
    @allure.step("Select the first available club")
    def select_first_club(self) -> None:
        """Select the first available club checkbox."""
        try:
            checkboxes = self.wait.until(
                lambda _: self._find_elements(self.CLUB_CHECKBOXES) or False
            )
