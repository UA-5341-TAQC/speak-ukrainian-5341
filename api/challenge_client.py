...
class ChallengeClient(BaseClient):
    ...
    def __init__(
        self, base_url: str = Config.BASE_API_URL, access_token: str | None = None
    ) -> None:
    ...
    def update_challenge_start_date(
        self,
        challenge_id: int,
        payload: dict[str, Any],
    ) -> requests.Response:
        """Update challenge start date."""
        return self._request("PUT", f"challenge/{challenge_id}/start/date", json=payload)
    ...
    def get_free_sort_number(self) -> int:
        ...
        used_sort_numbers = {challenge["sortNumber"] for challenge in challenges}
